...
const MyPosts = () => {
  ...
  const fetchMyPosts = async () => {
    const result = await fetch("http://localhost:4000/v1/my_posts", {
      method: "GET",
      credentials: "include",
      headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`http://localhost:4000/v1/delete_post/${id}`, {
        method: "DELETE",
        credentials: "include",
        headers: {
          Authorization: localStorage.getItem("token"),
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:4000/v1/edit_post/${editingPost._id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: localStorage.getItem("token"),
